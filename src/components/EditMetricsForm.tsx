...
export default function EditMetricsForm({ metrics, onSuccess, onCancel }: EditMetricsFormProps) {
  const [loading, setLoading] = useState(false);
  const [formData, setFormData] = useState({
    month: metrics.month,
    mrr: metrics.mrr.toString(),
    churn: metrics.churn.toString(),
    newRevenue: metrics.newRevenue.toString(),
    totalRevenue: metrics.totalRevenue.toString(),
    newCustomers: metrics.newCustomers.toString(),
    totalCustomers: metrics.totalCustomers.toString(),
    ltv: metrics.ltv.toString(),
    burnRateTechnology: metrics.burnRate.technology.toString(),
    burnRateSalaries: metrics.burnRate.salaries.toString(),
    burnRateProlabore: metrics.burnRate.prolabore.toString(),
    burnRateMarketing: metrics.burnRate.marketing.toString(),
    burnRateAdministrative: metrics.burnRate.administrative.toString(),
    burnRateOthers: metrics.burnRate.others.toString(),
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error('Error updating metrics:', error);
      toast.error("Erro inesperado ao atualizar métricas");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Editar Métricas - {formData.month}</CardTitle>
        <CardDescription>
          Edite as métricas da sua startup para o mês selecionado
        </CardDescription>
      </CardHeader>
      <CardContent>
